...
#include <stdio.h>
/**
 * print_ad - prints the address of an input variable
...
int print_ad(va_list arguments, char *buf, unsigned int ibuf)
{
	...
	binary = fill_binary_arra(binary, int_input, isnegative, 64);
	hexadecimal = malloc(sizeof(char) * (16 + 1));
	hexadecimal = fill_hex_arra(binary, hexadecimal, 0, 16);
	ibuf = handl_bu(buf, '0', ibuf);
	...
	for (first_digit = i = count = 0; hexadecimal[i]; i++)
	{
		...
		if (first_digit)
		{
			ibuf = handl_bu(buf, hexadecimal[i], ibuf);
			count++;
		}
	}
	free(binary);
